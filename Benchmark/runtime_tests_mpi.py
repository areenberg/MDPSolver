...
def randomMDP(nStates, nActions, sparsity):
    ...
    return rewards_mdpsolver, tranMatProbs_mdpsolver, tranMatColumns_mdpsolver, rewards_toolbox, transitions_toolbox
...
for ns in nStates:
    for sp in sparsity[nsidx]:
        for r in range(reps):
            ...
            rewards_mdpsolver, tranMatProbs_mdpsolver, tranMatColumns_mdpsolver, rewards_toolbox, transitions_toolbox = randomMDP(ns,nActions,sp)        

            #mdpsolver
            mdl = mdpsolver.model()
            mdl.mdp(discount=discount,
                rewards=rewards_mdpsolver,
                tranMatProbs=tranMatProbs_mdpsolver,
                tranMatColumns=tranMatColumns_mdpsolver)
            initVal = [0]*ns
            initPol = [0]*ns
            mdl.solve(algorithm="mpi",update="standard",parIterLim=parIterLim,tolerance=tolerance,initValueVector=initVal,initPolicy=initPol,
                      verbose=False,postProcessing=False,makeFinalCheck=False)
            runtime_mdpsolver = mdl.getRuntime()
            #print("mdpsolver:",runtime_mdpsolver)
        
            #mdptoolbox
            mdl_toolbox = mdptoolbox.mdp.PolicyIterationModified(transitions_toolbox,rewards_toolbox,discount,epsilon=tolerance,max_iter=parIterLim)
            ...
            #store results
            results.append([sp, ns, nActions, r, runtime_mdpsolver, runtime_toolbox,reldiff,'MPI'])
            iter += 1
            print(str(iter) + " / " + str(len(nStates)*len(sparsity)*reps),"ns:",ns,"sp:",sp,"rd:",reldiff)
    nsidx+=1

#save as csv-file
df = pd.DataFrame(results, columns=['Sparsity', 'NumStates', 'NumActions', 'Replication', 'RuntimeMDPSolver', 'RuntimeToolbox', 'RelativeDifference','Algorithm'])
df.to_csv('results/mdp_test_results_mpi.csv', index=False)