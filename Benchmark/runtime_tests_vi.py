...
for ns in nStates:
    for sp in sparsity[nsidx]:
        ...
        while(r<reps):
            ...
            initVal = [0]*ns
            initPol = [0]*ns
            mdl.solve(algorithm="vi",update="standard",tolerance=tolerance,initValueVector=initVal,initPolicy=initPol,
                      verbose=False,postProcessing=False,makeFinalCheck=False)
            runtime_mdpsolver = mdl.getRuntime()
            #print("mdpsolver:",runtime_mdpsolver)
        
            #mdptoolbox
            mdl_toolbox = mdptoolbox.mdp.ValueIteration(transitions_toolbox,rewards_toolbox,discount,epsilon=tolerance)
            ...
            if reldiff>0.0:
                #store results
                results.append([sp, ns, nActions, r, runtime_mdpsolver, runtime_toolbox,reldiff,'VI'])
                iter += 1
                r += 1
                print(str(iter) + " / " + str(len(nStates)*len(sparsity)*3*reps),"ns:",ns,"sp:",sp,"rd:",reldiff)
            else:
                print("Failed.")   
    nsidx+=1
