...
for ns in nStates:
    for sp in sparsity[nsidx]:
        for r in range(reps):
            ...
            initVal = [0]*ns
            initPol = [0]*ns
            mdl.solve(algorithm="pi",update="standard",tolerance=tolerance,initValueVector=initVal,initPolicy=initPol,
                      verbose=False,postProcessing=False,makeFinalCheck=False)
            runtime_mdpsolver = mdl.getRuntime()
            #print("mdpsolver:",runtime_mdpsolver)
        
            #mdptoolbox
            mdl_toolbox = mdptoolbox.mdp.PolicyIteration(transitions_toolbox,rewards_toolbox,discount)
            ...
            #store results
            results.append([sp, ns, nActions, r, runtime_mdpsolver, runtime_toolbox,reldiff,'PI'])
            iter += 1
...
df.to_csv('results/mdp_test_results_pi.csv', index=False)