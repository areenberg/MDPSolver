...
for ns in nStates:
    for sp in sparsity[nsidx]:
        for r in range(reps):
            ...
            # mdptoolbox
            mdl_toolbox = mdptoolbox.mdp.PolicyIteration(
                transitions_toolbox, rewards_toolbox, discount
            )
            ...
            # store results
            results.append(
                [sp, ns, nActions, r, runtime_mdpsolver, runtime_toolbox, reldiff, "PI"]
            )
            iter += 1
            print(
                str(iter) + " / " + str(len(nStates) * len(sparsity) * reps),
                "ns:",
                ns,
                "sp:",
                sp,
